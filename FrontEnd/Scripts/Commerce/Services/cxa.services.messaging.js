...
(function (root, factory) {
    ...
    MessageContext.PublishMessage = function (source, channel, message, data) {
        if (!hasHandlers(channel)) { return false; }
        var dispatchFunction = dispatch(source,channel, message, data)
        //run asyncronously
    ...
    MessageContext.PublishError = function (source,message, data) {
        ...
        var dispatchFunction = dispatch(source,MessageContext.StandardChannels.Error, message, data)
        //run asyncronously
        setTimeout(dispatchFunction, 0);
        return true;
    };
    ...
    MessageContext.UnSubscribeHandler = function (handlerId) {
        ...
        for (channel in Handlers) {
            if (Handlers.hasOwnProperty(channel)) {
                channelHandlers = MessageContext.Handlers[channel];
    ...
    MessageContext.ClearAllMessages = function () {
        MessageContext.PublishCommand(this, MessageContext.StandardCommands.ClearAll, null);
    };
