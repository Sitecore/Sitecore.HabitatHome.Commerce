...
(function (root, factory) {
    ...
    component.ProductSelectionChangedHandler = function (source, catalogName, productId, variantId, data) {
        ...
        var currentProductId = $(component.RootElement).find('#addtocart_productid').val();

        if (data && currentProductId === data.productId) {
            if (variantId) {
    ...
    {
        $(component.RootElement).find(".add-to-cart-btn").click(function (event) {
            event.preventDefault();

            var action = "";
            var data = "";
            var isBundle = $(component.RootElement).find("#addtocart_isbundle").val();
            var quantity = $(component.RootElement).find('.add-to-cart-qty-input').val();

            if (isBundle === "1") {
                component.bundleProductPayload.quantity = quantity;
                var data = component.bundleProductPayload.toJSONString();
                ...
                var catalogName = $(component.RootElement).find('#addtocart_catalogname').val();
                var productId = $(component.RootElement).find('#addtocart_productid').val();
                var variantId = $(component.RootElement).find('#addtocart_variantid').val();
