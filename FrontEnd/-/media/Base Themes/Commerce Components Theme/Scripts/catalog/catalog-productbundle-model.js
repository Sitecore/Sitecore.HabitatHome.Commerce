...
var BundledItemVariantCombo = function (variantInfo) {
  self = this;
...
var BundleVariantDefinition = function (bundleGroup) {
  ...
  self.readVariantInformation = function (variants) {
    ...
    $.each(variants, function () {
      ...
      $(this).find('.bundle-variant-value')
        .find('input')
        .each(function () {
      ...
      if (variantValueList.length > 0) {
        let variantInfo = { variantId: variantId, variantValueList: variantValueList, savingsMessage: savingsMessage, listPrice: listPrice, adjustedPrice: adjustedPrice, isOnSale: isOnSale };
        outerSelf.variantComboList.push(new BundledItemVariantCombo(variantInfo));
      } else {
...
var BundleGroup = function (viewModel, component, bundleGroupElement, id) {
  ...
  self.variantSelectionChange = function (selectBox, selectedValue) {
    ...
    if (currentBundleGroup.variantDefinition) {
  ...
  self.toggleClicked = function (bundleGroup, event) {
    ...
    if (isActive) {
      $(bundleLine).removeClass('active')
        .find('.bundle-group-body-container')
        .slideUp(200);
    } else {
      if (!hasNoVariants) {
        $(bundleLine).find('.bundle-group-body-container')
          .slideToggle(200);
        $(bundleLine).addClass('active');
      }
      $(bundleLine).siblings('.bundle-group')
        .removeClass('active')
        .find('.bundle-group-body-container')
        .slideUp(200);
    }
...

  {
    if ($(component.RootElement).length) {
      ...
      if ($(bundleGroupList).length) {
        ...
        var data = {};
        data.productId = $(outerSelf.component.RootElement).find('#bundle-component-product-id')
          .val();
        data.sourceProductVariantElement = outerSelf.component.RootElement;
        data.listPrice = $(component.RootElement).find("#bundle-component-product-listprice")
          .val();
        data.adjustedPrice = $(component.RootElement).find("#bundle-component-product-adjustedprice")
          .val();
        data.isOnSale = $(component.RootElement).find("#bundle-component-product-isonsale")
          .val() === "true";
        data.savingsMessage = $(component.RootElement).find("#bundle-component-product-savingsmessage")
          .val();
