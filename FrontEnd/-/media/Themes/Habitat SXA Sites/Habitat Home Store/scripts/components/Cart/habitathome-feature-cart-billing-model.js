...
function BillingDataViewModel() {
    ...
    function initModelData(data) {
        ...
        if (self.paymentClientToken() != null) {
            var validationButtonText = $(".validate-payment-btn").text();
            ...
            if (clientToken.length > 0) {
                braintree.setup(clientToken, 'dropin', {
                    ...
                    paymentMethodNonceReceived: function (event, nonce) {
                        ...
                        $(".validate-payment-btn").addClass("valid");
                        $(".validate-payment-btn").text(validationButtonText);

                    },
                    onError: function(data){
                        if(data.type == "VALIDATION")
                    {
                        console.log(data.details.invalidFields);
